...
- 那么`nD-->kD`就是找`k`个向量 ${u^{(1)}},{u^{(2)}} \ldots {u^{(k)}}$，所有数据投影到上面使投影误差最小
...
- 计算`协方差矩阵Σ`（Covariance Matrix）：$\Sigma = {1 \over m}\sum\limits_{i = 1}^n {{x^{(i)}}{{({x^{(i)}})}^T}}$
 - 注意这里的`Σ`和求和符号不同
...
- 多元高斯分布
 - $x \in {R^n}$，并不是建立`p(x1),p(x2)...p(xn)`，而是统一建立`p(x)`
 - 其中参数：$\mu  \in {R^n},\Sigma  \in {R^{n \times {\rm{n}}}}$,`Σ`为**协方差矩阵**
